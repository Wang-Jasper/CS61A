...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    "*** YOUR CODE HERE ***"
    index = 1
    current_gene_list = lnk_of_genes
    while current_gene_list is not Link.empty:
        gene = current_gene_list.first
        current_node = gene
        aug_node = None

        while current_node is not Link.empty:
            if current_node.first == "AUG":
                aug_node = current_node
                break
            current_node = current_node.rest
        
        if aug_node is not None:
            original_rest = aug_node.rest
            new_rest = original_rest
            for _ in range(index):
                new_rest = Link(insert, new_rest)
            aug_node.rest = new_rest
        current_gene_list = current_gene_list.rest
        index += 1
...
def transcribe(dna):
    ...
    return [''.join([{'A': 'U', 'T': 'A', 'G': 'C', 'C': 'G'}[nucleotide] for nucleotide in dna][i:i+3]) for i in range(0, len(dna), 3)]
